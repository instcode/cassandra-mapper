...
object ReflectionUtil {
  ...
  def create[T: TypeTag](args: Array[Object]): T = {
    val tpe = implicitly[TypeTag[T]].tpe
    val ctorSymbol = Reflect.methodSymbol(tpe)
    val clazz = cm classSymbol cm.runtimeClass(tpe)
    val clm = cm reflectClass clazz
    val mm = clm.reflectConstructor(ctorSymbol)
    mm.apply(args: _*).asInstanceOf[T]
  }
}